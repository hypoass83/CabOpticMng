namespace FatSod.DataContext.Migrations
{
    ...
    public partial class FCT_GenerateReportOperationPeriodeOfDayBank : DbMigration
    {
        public override void Up()
        {
            Sql(@"ALTER FUNCTION [dbo].[GenerateReportOperationPeriodeOfDayBank]
            (
            @To DateTime,
            @At DateTime
            )
            RETURNS @TableGenerateReportOperationPeriodeOfDayBank TABLE 
            (
            ...
            As
            Begin
	            Declare @dateop DATEtIME,@RptCashDayOperationID Int,@rowtill numeric,@COUNTSO NUMERIC,@BudgetAllocatedID Numeric,
	            ...
	            @OpeningCashAmount Float,@ClosingCashAmount Float,@PaymentMethod Varchar(50),@CashRegisterName Varchar(250)
	            ...
	            DECLARE @BankID NUMERIC

	            SET @dateop=@To
	            WHILE (@dateop <= @At)
	            BEGIN

		            --recuperation des bank ds la bd
		            set @BankID=NULL
		            declare bank_Cursor CURSOR FOR
		            SELECT b.ID FROM dbo.Banks b inner join dbo.PaymentMethods p on b.ID=p.ID
		            OPEN bank_Cursor
		            ...
		            BEGIN 
			            ...
			            select @PaymentMethod=p.Code,@CashRegisterName=p.Name+' '+p.Code from dbo.Banks t inner join dbo.PaymentMethods p on t.ID=p.ID where t.ID=@BankID
			            ...
			            select @OpeningCashAmount=0,@ClosingCashAmount=0
				            ...
				            ON CSL.SliceID=SL.SliceID WHERE SL.SliceDate =@dateop AND SL.PaymentMethodID=@BankID AND CSL.isDeposit=0
				            OPEN ventes_Cursor;  
				            ...
				            BEGIn 
					            ...
					            SET @COUNTSO=0
					            --SELECT @COUNTSO=COUNT(*) FROM DBO.SpecialOrders SO INNER JOIN DBO.CustomerOrders CO
					            --ON SO.CustomerOrderID=CO.CustomerOrderID WHERE CustomerOrderNumber=@SaleReceiptNumber
					            ...
					            if (@SliceAmount>0)
					            begin
						            Insert Into @TableGenerateReportOperationPeriodeOfDayBank(OperationDate,BeginDate,
				            ...
				            FROM dbo.AllDeposits dp INNER JOIN dbo.GlobalPeople c on dp.CustomerID=c.GlobalPersonID  WHERE dp.AllDepositDate =@dateop AND dp.PaymentMethodID=@BankID
				            OPEN Deposit_Cursor;  
				            ...
				            FROM dbo.BudgetConsumptions bc WHERE bc.PaymentDate =@dateop AND bc.PaymentMethodID=@BankID 
				            OPEN Budget_Cursor;  
				            ...
				            WHERE sl.SliceDate =@dateop AND sl.PaymentMethodID=@BankID 
