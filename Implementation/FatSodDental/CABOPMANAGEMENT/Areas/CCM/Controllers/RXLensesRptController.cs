...
namespace CABOPMANAGEMENT.Areas.CCM.Controllers
{
    ...
    public class RXLensesRptController : BaseController
    {        
        ...
        public JsonResult GetRXOrders(DateTime BeginDate, DateTime EndDate)
        {
            var model = new
            {
                data = from c in db.CumulSaleAndBills.Where(c => !c.isReturn &&
                       ...
                       new
                       {
                           ...
                           Status = c.IsReceived ? "Received" : (c.IsOrdered ? "Ordered" : "Pending"),
                           OrderDate = c.OrderDate.HasValue ? c.OrderDate.Value.ToString("yyyy-MM-dd") : "",
                           ReceivedDate = c.SpecialOrderReceptionDateHeure.HasValue ? c.SpecialOrderReceptionDateHeure.Value.ToString("yyyy-MM-dd") : "",
                           OrderBy = c.OrderBy != null ? c.OrderBy.UserFullName : "",
                           ControlledBy = c.OrderController != null ? c.OrderController.UserFullName : "",
                           ReceivedBy = c.Receiver != null ? c.Receiver.UserFullName : "",
        ...
        public JsonResult GetStockOrders(DateTime BeginDate, DateTime EndDate)
        {
            var model = new
            {
                data = from c in db.CumulSaleAndBills.Where(c => !c.isReturn &&
                                        ...
                                        c.CumulSaleAndBillLines.Any(line => line.StockType == StockType.STOCK_ORDER)
                       ).ToList()
                       select
                       new
                       {
