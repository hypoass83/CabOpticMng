...
namespace CABOPMANAGEMENT.Areas.Sale.Controllers
{
    ...
    public class DIRECTBILLController : BaseProformaController
    {
        ...
        public JsonResult InitProductDetail(int ProductLocalizationID)
        {
            ...
            if (ProductLocalizationID > 0)
            {
                ...
                Session["CurrentProduct"] = prodLocalization.ProductLabel + prodLocalization.Marque;
        ...
        public JsonResult LoadCompteurFacture(int AssureurID)
        {
            ...
            if (assure != null)
            {
                _InfoList.Add(new
                {
                    CompteurFacture = assure.CompteurFacture
                });
            }
        ...
        public bool DoYes(SpecialLensModel slm)
        {
            ...
            try
            {
                ...
                foreach (CustomerOrderLine customerOrderLine in cols)
                {
                    ...
                    if (!(customerOrderLine.Product is OrderLens) && !(customerOrderLine.Product is Lens))
                    {
        ...
        public JsonResult CustomerOrderLines()
        {
            ...
            var model = new
            {
                data = from c in customerOrderLines
                select
                new
                {
                LineID = c.LineID,
        ...
        public JsonResult AddAssureSale(SpecialLensModel slm, CustomerOrder customerOrder, string heureVente, int spray = 0, int boitier = 0)
        {
            bool status = false;
            string Message = "";
            try
            {
                ...
                customerOrder.VatRate = CodeValue.Accounting.ParamInitAcct.VATRATE;
                customerOrder.IsDelivered = true;
                customerOrder.BillState = StatutFacture.Validated;
                ...
                var PlafondWithRemiseAmount = customerOrder.Plafond; // - (customerOrder.RemiseAssurance * customerOrder.Plafond) / 100;
                customerOrder.Plafond = PlafondWithRemiseAmount;
                
                customerOrder.OperatorID = this.SessionGlobalPersonID;
                int CustomerId = _CustomerOrderRepository.SaveDirectChanges(customerOrder, heureVente, SessionGlobalPersonID,spray,boitier, true).CustomerOrderID;
                
                Session["Receipt_CustomerOrderID"] = CustomerId;
        ...
        public JsonResult GetQuantityStock(string Localization, string CurrentProduct, string NumeroSerie)
        {
            ...
            if ((Localization != null && CurrentProduct != null) && (Localization.Length > 0 && CurrentProduct.Length > 0))
            {
                ...
                if (idLoc > 0 && idProd > 0)
                {
                    ...
                    Session["CurrentProduct"] = (!(productcat.isSerialNumberNull)) ? productInStock.ProductCode : productInStock.ProductCode + " " +productInStock.Marque;
