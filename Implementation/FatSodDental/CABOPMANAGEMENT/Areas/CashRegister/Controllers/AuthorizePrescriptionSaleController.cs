...
namespace CABOPMANAGEMENT.Areas.CashRegister.Controllers
{
    ...
    public class AuthorizePrescriptionSaleController : BaseController
    {
        // GET: CashRegister/AuthorizePrescriptionSale
        ...
        private const string CONTROLLER_NAME = "CashRegister/AuthorizePrescriptionSale";
        ...
        //Construcitor
        public AuthorizePrescriptionSaleController(
        ...
        public JsonResult LoadCustomers(string filter)
        {
            ...
            var list = (from l1 in db.Customers
                        ...
                        where !l1.IsBillCustomer && lc.isPrescritionValidate && !lcp.isAuthoriseSale && l1.Name.Contains(filter.ToLower())
                        select new
                        {
                            GlobalPersonID = l1.GlobalPersonID,
                            Name = l1.Name,
                            Description=l1.Description,
                            PrescriptionID = lp.ConsultDilPrescID // .PrescriptionID
                        }).ToList();
        ...
        public JsonResult GetAllSerialNumber(string filter, int DepartureLocalizationID = 0)
        {
            ...
            foreach (var prodLocalization in res)
            {
                customersList.Add(new
                {
                    ...
                    LocalizationID = prodLocalization.LocalizationID,
                    ProductID = prodLocalization.ProductID,
                    BarCode = prodLocalization.BarCode,
        ...
        public JsonResult InitTrnNumber(int CustomerID,int PrescriptionID)
        {
            List<object> _CommandList = new List<object>();

            var lstPrescCusto = (from l1 in db.viewCustomers
                        join lc in db.Consultations on l1.GlobalPersonID equals lc.CustomerID
                        join lp in db.ConsultDilPrescs on lc.ConsultationID equals lp.ConsultationID
                        join lcp in db.ConsultLensPrescriptions on lp.ConsultDilPrescID equals lcp.ConsultDilPrescID
                        where lc.isPrescritionValidate && l1.GlobalPersonID == CustomerID && lcp.ConsultDilPrescID == PrescriptionID
                        select new
                        {
                            PrescriptionID = lp.ConsultDilPrescID,
                            CustomerNumber = l1.CustomerNumber,
                            CustomerID = l1.GlobalPersonID,
                            Remarque = (l1.AdressPhoneNumber != null) ? l1.AdressPhoneNumber : "",
            ...
            string LensCategoryCode = (cat != null) ? cat.CategoryCode : "";
            string TypeLens = (cat != null) ? cat.TypeLens : ""; 

            _CommandList.Add(new
            {
                PrescriptionID = lstPrescCusto.PrescriptionID,
                CustomerNumber = lstPrescCusto.CustomerNumber,
                CustomerID = lstPrescCusto.CustomerID,
                ConsultationID=lstPrescCusto.ConsultationID,
        ...
        //[HttpPost]
        public JsonResult AddAuthoriseSale(SpecialLensModel slm, AuthoriseSale currentSale, string heureVente, int spray = 0, int boitier = 0)
        {
            bool status = false;
            string Message = ""; 
            try
        ...
        private List<AuthoriseSale> ModelCommand
        {
            get
            {
                ...
                db.AuthoriseSales.Where(c => !c.IsDelivered && !c.IsSpecialOrder && !c.IsDilatation)
                .ToList().ForEach(c =>
                {
        ...
        public JsonResult DeleteCommand(int ID)
        {
            ...
            try
            {
                ...
                int? consultDilPrescID = custord.ConsultDilPrescID;
                ...
                if (consultDilPrescID != null && consultDilPrescID.HasValue && consultDilPrescID.Value > 0)
                {
                    ConsultLensPrescription consultprescription = db.ConsultLensPrescriptions.Where(c => c.ConsultDilPrescID == consultDilPrescID).FirstOrDefault();
                    consultprescription.isAuthoriseSale = false;
                    db.SaveChanges();
                }
