...
namespace VALDOZMANAGEMENT.Areas.CashRegister.Controllers
{
    [Authorize(Order = 1)]
    ...
    public class AuthorizeSaleController : BaseController
    {
            ...
            try
            {
                ...
                foreach (AuthoriseSaleLine authosaleLine in cols)
                {
                    ...
                    if ((authosaleLine.Product is GenericProduct))
                    {
                        ...
                        res = this.CheckQty(authosaleLine.LocalizationID, authosaleLine.Product.ProductID, authosaleLine.NumeroSerie, authosaleLine.LineQuantity, spray, boitier);
                        if (!res)
                            return res;
                    }
        ...
        public bool CheckQty(int LocalizationID, int? ProductID, string NumeroSerie, double LineQuantity, int spray, int boitier)
        {
            ...
            try
            {
                ...
                //{
                double currentQteEnStock = this.GetQuantityStock(LocalizationID.ToString(), ProductID.ToString(), NumeroSerie);
                //}
        ...
        public JsonResult AddAuthoriseSale(SpecialLensModel slm, AuthoriseSale currentSale,  string heureVente, int spray = 0, int boitier = 0)
        {
            ...
            try
            {
                ...
                if (currentSale.AuthoriseSaleLines.Count > 0)
                {
                    ...
                    int SaleID = _authoriseSaleRepository.SaveChanges(currentSale, heureVente, SessionGlobalPersonID).AuthoriseSaleID;
