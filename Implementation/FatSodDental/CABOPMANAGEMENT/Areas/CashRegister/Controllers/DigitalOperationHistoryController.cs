...
namespace CABOPMANAGEMENT.Areas.CashRegister.Controllers
{
    ...
    public class DigitalOperationHistoryController : BaseController
    {
        private const string CONTROLLER_NAME = "CashRegister/DigitaloperationHistory";
        //*********************
        ...
        public DigitalOperationHistoryController(
        ...
        public ActionResult Index()
        {
         ...
         public JsonResult DigitalPaymentMethods(int BranchID)
        {

            List<object> model = new List<object>();

            db.DigitalPaymentMethods.Where(dpm => dpm.BranchID == BranchID).ToList().ForEach(pm =>
            {
                model.Add(
                        new
                        {
        ...
        public JsonResult LoadDigitalOperation(int BranchID, DateTime? Begindate, DateTime? EndDate, int? DigitalPaymentMethodId)
        {
            var res = new List<object>();
            var digitalOperationHistories = new List<RptDigitalOperation>();

            var slices = DigitalSlices(BranchID, Begindate, EndDate, DigitalPaymentMethodId);
            var allDeposits = DigitalAllDeposits(BranchID, Begindate, EndDate, DigitalPaymentMethodId);
            // var deposits = DigitalDeposits(BranchID, Begindate, EndDate, DigitalPaymentMethodId);
            var returnSales = DigitalReturns(BranchID, Begindate, EndDate, DigitalPaymentMethodId);

            if (slices.Count > 0)
            {
                res.AddRange(slices);
                digitalOperationHistories.AddRange(slices);
            }

            if (allDeposits.Count > 0)
            {
                res.AddRange(allDeposits);
                digitalOperationHistories.AddRange(allDeposits);
            }

            /*if (deposits.Count > 0)
            {
                res.AddRange(deposits);
                digitalOperationHistories.AddRange(deposits);
            }*/

            if (returnSales.Count>0)
            {
                res.AddRange(returnSales);
                digitalOperationHistories.AddRange(returnSales);
            }
            ...
            Session["DigitalOperationHistories"] = digitalOperationHistories;
        ...
        public JsonResult GetCashOperationTotal(int BranchID, DateTime? Begindate, DateTime? EndDate, int? tillID)
        {
            ...
            var digitalOperationHistories = ((List<RptDigitalOperation>)Session["DigitalOperationHistories"]);

            var TotalInput = digitalOperationHistories.Where(a => a.Type == "INPUT").Sum(b => b.Amount);
            var TotalSortie = digitalOperationHistories.Where(a => a.Type == "OUTPUT").Sum(b => b.Amount);
        ...
        #region SLICES
        public List<RptDigitalOperation> DigitalSlices(int BranchID, DateTime? beginDate, DateTime? endDate, int? digitalPaymentMethodId)
        {
            var res = new List<RptDigitalOperation>();
            var salesOfTheDayIds = db.DigitalPaymentSales.Where(s => s.SaleDate >= beginDate && s.SaleDate <= endDate
                                             ).Select(a => a.TransactionIdentifier).ToList();

            if (digitalPaymentMethodId.HasValue && digitalPaymentMethodId.Value > 0)
            {
                var a = db.CustomerSlices.Where(s => salesOfTheDayIds.Contains(s.TransactionIdentifier) && s.PaymentMethodID == digitalPaymentMethodId.Value &&
                                                    s.SliceDate >= beginDate && s.SliceDate <= endDate
                                             ).ToList();

                res = a.Select(Convert).ToList();
            }
            else
            {
                var a = db.CustomerSlices.Where(s => salesOfTheDayIds.Contains(s.TransactionIdentifier) && s.PaymentMethod is DigitalPaymentMethod &&
                                                   s.SliceDate >= beginDate && s.SliceDate <= endDate
                                              ).ToList();
        ...
        public RptDigitalOperation Convert(CustomerSlice slice)
        {
            var res = new RptDigitalOperation()
            {
                AccountManager = slice.DigitalAccountManager?.FullName,
                AccountName = (slice.PaymentMethod as DigitalPaymentMethod)?.FullName,
                Amount = slice.SliceAmount,
                BranchName = slice.Sale?.Branch?.BranchName,
                TransactionCode = slice.TransactionIdentifier,
                OperationDate = slice.SliceDate.ToString("yyyy-MM-dd"),
                Intervenant = slice.Sale?.Customer?.FullName,
                Operation = slice.isDeposit ? "DEPOSIT" : "SALE",
                Operator = slice.Operator?.FullName,
                Reference = slice.Reference,
                RptDigitalOperationID = slice.SliceID,
                Type = "INPUT"
            };
            return res;
        }
        ...
        #region ALL DEPOSITS
        public List<RptDigitalOperation> DigitalAllDeposits(int BranchID, DateTime? beginDate, DateTime? endDate, int? digitalPaymentMethodId)
        {
            var res = new List<RptDigitalOperation>();
            if (digitalPaymentMethodId.HasValue && digitalPaymentMethodId.Value > 0)
            {
                var a = db.AllDeposits.Where(s => s.PaymentMethodID == digitalPaymentMethodId.Value &&
                                                    s.AllDepositDate >= beginDate && s.AllDepositDate <= endDate
                                          ).ToList();
                res = a.Select(Convert).ToList();
            }
            else
            {
                var a = db.AllDeposits.Where(s => s.PaymentMethod is DigitalPaymentMethod &&
                                                s.AllDepositDate >= beginDate &&
                                                s.AllDepositDate <= endDate
                                          ).ToList();
                res = a.Select(Convert).ToList();
            }
        ...
        public RptDigitalOperation Convert(AllDeposit deposit)
        {
            var res = new RptDigitalOperation()
            {
                AccountManager = deposit.DigitalAccountManager.FullName,
                AccountName = (deposit.PaymentMethod as DigitalPaymentMethod).FullName,
                Amount = deposit.Amount,
                BranchName = deposit.Branch?.BranchName,
                OperationDate = deposit.AllDepositDate.ToString("yyy-MM-dd"),
                Intervenant = deposit.Representant,
                TransactionCode  = deposit.TransactionIdentifier,
                Operation = "DEPOSIT",
                Operator = deposit.Operator?.FullName,
                Reference = deposit.AllDepositReference,
                RptDigitalOperationID = deposit.AllDepositID,
                Type = "INPUT"
            };
            return res;
        }
        ...
        #region ALL DEPOSITS
        public List<RptDigitalOperation> DigitalDeposits(int BranchID, DateTime? beginDate, DateTime? endDate, int? digitalPaymentMethodId)
        {
            var res = new List<RptDigitalOperation>();
            if (digitalPaymentMethodId.HasValue && digitalPaymentMethodId.Value > 0)
            {
                var a = db.Deposits.Where(s => s.PaymentMethodID == digitalPaymentMethodId.Value &&
                                                    s.DepositDate >= beginDate && s.DepositDate <= endDate
                                        );
                res = a.Select(Convert).ToList();
            }
            else
            {
                var a = db.Deposits.Where(s => s.PaymentMethod1 is DigitalPaymentMethod && 
                                             s.DepositDate >= beginDate && s.DepositDate <= endDate
                                       );
        ...
        public RptDigitalOperation Convert(Deposit deposit)
        {
            var res = new RptDigitalOperation()
            {
                AccountManager = deposit.DigitalAccountManager.FullName,
                AccountName = (deposit.PaymentMethod1 as DigitalPaymentMethod).FullName,
                Amount = deposit.Amount,
                BranchName = deposit.SavingAccount.Branch.BranchName,
                Date = deposit.DepositDate,
                Intervenant = deposit.Representant,
                Operation = "DEPOSIT",
                Operator = deposit.Operator?.FullName,
                Reference = deposit.DepositReference,
                RptDigitalOperationID = deposit.DepositID,
                Type = "INPUT"
            };
            return res;
        }
        ...
        public List<RptDigitalOperation> DigitalReturns(int BranchID, DateTime? beginDate, DateTime? endDate, int? digitalPaymentMethodId)
        {
            var res = new List<RptDigitalOperation>();
            /*var salesOfTheDayIds = db.DigitalPaymentSales.Where(s => s.SaleDate >= beginDate && s.SaleDate <= endDate
                                             ).Select(a => a.TransactionIdentifier).ToList();
            */
            if (digitalPaymentMethodId.HasValue && digitalPaymentMethodId.Value > 0)
            {
                var a = db.CustomerReturnSlices.Where(s => /*salesOfTheDayIds.Contains(s.TransactionIdentifier) &&*/ s.PaymentMethodID == digitalPaymentMethodId.Value &&
                                                    s.SliceDate >= beginDate && s.SliceDate <= endDate
                                             ).ToList();
            ...
            {
                var a = db.CustomerReturnSlices.Where(s => /*salesOfTheDayIds.Contains(s.TransactionIdentifier) &&*/ s.PaymentMethod is DigitalPaymentMethod &&
                                                   s.SliceDate >= beginDate && s.SliceDate <= endDate
                                              ).ToList();
        ...
        public RptDigitalOperation Convert(CustomerReturnSlice returnP)
        {
            var res = new RptDigitalOperation()
            {
                AccountManager = returnP.DigitalAccountManager?.FullName,
                AccountName = (returnP.PaymentMethod as DigitalPaymentMethod).FullName,
                Amount = returnP.SliceAmount,
                BranchName = returnP.CustomerReturn?.Sale?.Branch?.BranchName,
                OperationDate = returnP.SliceDate.ToString("yyy-MM-dd"),
                Intervenant = returnP.Representant,
                TransactionCode = returnP.TransactionIdentifier,
                Operation = "SALE RETURN",
                Operator = returnP.Operator?.FullName,
                Reference = returnP.CustomerReturn?.Sale?.SaleReceiptNumber,
                RptDigitalOperationID = returnP.SliceID,
                Type = "OUTPUT"
            };
            return res;
        }
