...
namespace VALDOZMANAGEMENT.Areas.CRM.Controllers
{
    ...
    public class ReceiveSpecialOrderController : BaseController
    {
        

        private const string CONTROLLER_NAME = "CRM/ReceiveSpecialOrder";
        ...
        public ReceiveSpecialOrderController(
        ...
        public JsonResult ModelSaleValidate()
        {
            ...
            List<CumulSaleAndBill> lstSale = db.CumulSaleAndBills.Where(c => c.IsRendezVous && !c.IsProductReveive && !c.isReturn).ToList();
            var model = new
            {
                data = from c in lstSale.ToList()
                       ...
                       new
                       {
                           ...
                           TotalPriceHT =c.CumulSaleAndBillLines.Sum(sl=>sl.PurchaseLineUnitPrice), // Util.ExtraPrices(c.CumulSaleAndBillLines.Select(sl => sl.LineAmount).Sum(),
        ...
        public JsonResult InitializeCommandFields(int ID)
        {
            ...
            try
            {
                ...
                CumulSaleAndBill autSale = (from c in db.CumulSaleAndBills
                                            where c.CumulSaleAndBillID == ID && c.IsRendezVous && !c.IsProductReveive 
                ...
                foreach (CumulSaleAndBillLine authosaleLine in SaleLines)
                {
                    ...
                    else
                    {
                        ...
                        LensPrice += (authosaleLine.PurchaseLineUnitPrice); //(autSale.SaleID != null) ? (authosaleLine.LineUnitPrice * 2) : (authosaleLine.LineUnitPrice);
                        LensLineQuantity += authosaleLine.LineQuantity;
                        FinalLensPrice += authosaleLine.PurchaseLineUnitPrice;
        ...
        public JsonResult ValideReceiveSpecialOrder(SpecialLensModel slm, CumulSaleAndBill currentSale)
        {
            bool status = false;
            string Message = "";
            try
            {
                ...
                if (currentSale.CumulSaleAndBillLines.Count > 0)
                {
                    ...
                    int CumulSaleAndBillID = _saleRepository.ValideReceiveSpecialOrder(currentSale, SessionGlobalPersonID).CumulSaleAndBillID;
                ...
                status = true;
                Message = Resources.Success + " - " + Resources.ReceiveSpecialOrder;
            }
            catch (Exception e)
            {
                status = false;
