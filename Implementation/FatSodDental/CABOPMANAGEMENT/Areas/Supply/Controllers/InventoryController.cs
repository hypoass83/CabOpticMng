...
namespace CABOPMANAGEMENT.Areas.Supply.Controllers
{
    ...
    public class InventoryController : BaseController
    {
            ...
            try
            {
                foreach (InventoryLines inLines in inventoryLines)
                {
                    ...
                    if (prodLoc.ProductLocalizationID > 0)
                    {
                        ...
                        _plRepository.UpdateProductLocalization(prodLoc, serverdate, inLines.ProductLocalizationStockQuantity);
                    }
                }
        ...
        public JsonResult EditLine(int ProductLocalizationID, int ProductLocalizationStockQuantity, int ProductLocalizationSafetyStockQuantity)
        {
            ...
            try
            {
                ...
                ProductLocalization prodLoc = new ProductLocalization();
                prodLoc = _plRepository.Find(ProductLocalizationID);
                DateTime serverdate = SessionBusinessDay(null).BDDateOperation;
                if (prodLoc.ProductLocalizationID > 0)
                {
                    prodLoc.ProductLocalizationSafetyStockQuantity = Convert.ToDouble(ProductLocalizationSafetyStockQuantity);
                    //prodLoc.ProductLocalizationStockQuantity = Convert.ToDouble(ProductLocalizationStockQuantity);*/
                    prodLoc.inventoryReason = "Stock Inventory";
                    prodLoc.RegisteredByID = SessionGlobalPersonID;
                    prodLoc.Description = "Stock Inventory";
                    _plRepository.UpdateProductLocalization(prodLoc, serverdate, ProductLocalizationStockQuantity);
                }
        ...
        //[HttpPost]
        public JsonResult Edit(string[] line)
        {
            int id = 0;
            string newSeuil = "";
            string newValue = "";
            ...
            try
            {
                ...
                {
                    string[] listline = line[i].Split(';');
                    var idLine = listline[0];

                    //decomposition de line
                    if (idLine.Trim() == "") continue;
                    if (idLine == "ID")
                    {
                        status = true;
                        Message = "";
                    ...
                    {
                        id = Convert.ToInt32(listline[0]);
                        newValue = listline[4];
                        //newSeuil = listline[4];
                        ProductLocalization prodLoc = new ProductLocalization();
                        prodLoc = _plRepository.Find(id);
                        DateTime serverdate = SessionBusinessDay(null).BDDateOperation;
                        if (prodLoc.ProductLocalizationID > 0)
                        {
                            prodLoc.ProductLocalizationSafetyStockQuantity = 1d;
                ...
                status = true;
                Message = "Imported -" + line.Length + "- rows successfully!";
                return new JsonResult { Data = new { status = status, Message = Message } };
            }
            catch (Exception e)
            {
