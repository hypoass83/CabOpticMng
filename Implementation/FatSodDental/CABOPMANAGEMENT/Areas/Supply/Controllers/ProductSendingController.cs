...
namespace CABOPMANAGEMENT.Areas.Supply.Controllers
{
    ...
    public class ProductSendingController : BaseController
    {
        //Current Controller and current page
        private bool isLens = false;
        private const string CONTROLLER_NAME = "Supply/" + CodeValue.Supply.PRODUCT_TRANSFERT_SENDING;
        private const string VIEW_NAME = "Index";
        private IInventoryDirectory inventoryDirectoryRepository;
        private readonly ITransfert transfertRepository;
        ...
        public JsonResult ChangeBusDay(int? BranchID)
        {
            ...
            if (BranchID.HasValue && BranchID.Value > 0)
            {
                ...
                string trnnum = _transactNumbeRepository.returnTransactNumber("TRAN", businessDay);
                list.Add(
                new
                {
                    ProductTransfertDate = businessDay.BDDateOperation.ToString("yyyy-MM-dd"),
                    ProductTransfertReference = trnnum
                }
                );
            }
        ...
        //[HttpPost]
        public JsonResult SendProduct(ProductTransfert productTransfert)
        {
            bool status = false;
            string Message = "";
            try
            {
                ...
                if (InventoryDirectoryLines != null && InventoryDirectoryLines.Count() > 0)
                {
                    productTransfert.ProductTransfertLines = InventoryDirectoryLines.Select(Convert).ToList();
                    transfertRepository.Sending(productTransfert, SessionGlobalPersonID);
                ...
                {
                    status = false;
                    Message = Resources.AlertError + " - " + "Please Add Products to your Transfert";
                }
        ...
        //[HttpPost]
        public JsonResult DeleteProductSending(int InventoryDirectoryID)
        {
            bool status = false;
            string Message = "";
            try
        ...
        public JsonResult BarCodeScanned(string BarCode)
        {
            ...
            try
            {
                ...
                var invDirLine = new InventoryDirectoryLine()
                {
                ...
                return AddInventoryDirectoryLine(invDirLine, 1);
            }
            catch (Exception e)
            {
                status = false;
        ...
        public ProductTransfertLine Convert(InventoryDirectoryLine line)
        {
            var departureStock = db.ProductLocalizations.FirstOrDefault(s => s.LocalizationID == line.LocalizationID &&
                                                                               s.ProductID == line.ProductID);
            var res = new ProductTransfertLine()
            {
                Quantity = int.Parse(line.NewStockQuantity.Value.ToString()),
                UnitPrice = line.AveragePurchasePrice,
                DepartureStockId = departureStock.ProductLocalizationID
            };
