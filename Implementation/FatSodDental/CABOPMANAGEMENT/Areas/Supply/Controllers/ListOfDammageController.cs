...
namespace CABOPMANAGEMENT.Areas.Supply.Controllers
{
    ...
    public class ListOfDammageController : BaseController
    {
        //Current Controller and current page
        private const string CONTROLLER_NAME = "Supply/ListOfDammage";
        ...
        public ListOfDammageController(
        ...
        public JsonResult chargeSolde(int BranchID, DateTime Bdate, DateTime Edate)
        {
            ...
            List<ProductDamage> lstCustoSaleAmt = new List<ProductDamage>();
            double Totaldebit = 0d;

            //int TotalSO = 0;


            lstCustoSaleAmt = _productDamageRepository.FindAll.Where(so => (so.ProductDamageDate.Date >= Bdate.Date && so.ProductDamageDate.Date <= Edate) && so.BranchID == BranchID).ToList();


            foreach (ProductDamage c in lstCustoSaleAmt)
            {

                double CustomerOrderTotalPrice = c.ProductDamageLines.Select(sl => (sl.LineQuantity * sl.LineUnitPrice)).Sum();
        ...
        public JsonResult ModelProductDamage(int BranchID, DateTime Bdate, DateTime Edate)
        {
            ...
            //produit de verre
            var query = db.ProductDamages.Join(db.ProductDamageLines, cr => cr.ProductDamageID, crln => crln.ProductDamageID,
                (cr, crln) => new { cr, crln })
                .Where(crsaline => (crsaline.cr.ProductDamageDate >= Bdate.Date && crsaline.cr.ProductDamageDate <= Edate.Date) && crsaline.cr.BranchID == BranchID)
                .Select(s => new
                {
                    ProductDamageDate = s.cr.ProductDamageDate,
                    ProductDamageID = s.cr.ProductDamageID,
                    ProductDamageReason = s.crln.ProductDamageReason,
                    LineQuantity = s.crln.LineQuantity,
                    LineUnitPrice = s.crln.LineUnitPrice,
                    Localization = s.crln.Localization,
                    Product = s.crln.Product,
                    Branch = s.cr.Branch,
                    ProductDamageReference = s.cr.ProductDamageReference,
            ...
            foreach (var c in query.OrderBy(cr => cr.ProductDamageDate).ThenBy(cr => cr.Product.ProductCode))
            {
                double giftAmnt = c.LineQuantity * c.LineUnitPrice;
                list.Add(
                    new RptReturnSale
                    {
                        RptReturnSaleID = c.ProductDamageID,
                        Agence = c.Branch.BranchCode,
                        LibAgence = c.Branch.BranchDescription,
                        CustomerReturnCauses = c.ProductDamageReason,
                        ...
                        CustomerReturnDate = c.ProductDamageDate,
                        CodeClient = c.ProductDamageReference,
