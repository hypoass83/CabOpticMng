...
namespace FatSodDental.UI.Tools
{
    public static class CultureHelper
    {
        ...
        public static string GetImplementedCulture(string name)
        {
            ...
            if (_validCultures.Where(l => l.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() == 0)
                return GetDefaultCulture(); // return Default culture if it is invalid
            // if it is implemented, accept it
            if (_cultures.Where(l => l.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
                return name; // accept it
            // Find a close match. For example, if you have "en-US" defined and the user requests "en-GB", 
            // the function will return closes match that is "en-US" because at least the language is the same (ie English)  
            var n = GetNeutralCulture(name);
            foreach (var l in _cultures)
                if (l.StartsWith(n))
                    return l;*/
            // else 
            if (name == "fr")
        ...
        public static void Changer_langue(String langue)
        {
            if (langue != "en-US")
            {
