...
namespace FatSodDental.UI.Areas.Sale.Controllers
{
    public class CustomerHistoryController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            //if (!LoadAction.isAutoriseToGetSubMenu(SessionProfileID, CodeValue.Sale.Report.CODESUBMENU2, db))
            //{
        ...
        private void chargeSolde(AccountOperation acctop)
        {
            ...
            List<AccountOperation> listAccOp = db.AccountOperations.Where(ao => ao.BranchID == acctop.BranchID && ao.DeviseID == acctop.DeviseID  && ao.AccountTierID == AcctNo).ToList();
            if (listAccOp != null)
            {
        ...
        public void Reset()
        {
            this.GetCmp<FormPanel>("RptCustomerHistory").Reset(true);
            this.PartialReset();
        }
        ...
        private List<object> ModelAcctOp(AccountOperation acctop) //, string BeginDate, string EndDate)
        {
            ...
            List<AccountOperation> listAccOp = db.AccountOperations.Where(ao => ao.BranchID == acctop.BranchID && ao.DeviseID == acctop.DeviseID && (ao.DateOperation >= Bdate && ao.DateOperation <= Edate) && ao.AccountTierID == AcctNo).ToList();
        ...
        private List<object> ModelRepAcctOp(AccountOperation acctop)
        {
            ...
            List<AccountOperation> listRepAccOp = db.AccountOperations.Where(ao => ao.BranchID == acctop.BranchID && ao.DeviseID == acctop.DeviseID && (ao.DateOperation < Bdate) && ao.AccountTierID == AcctNo).ToList();
        ...
        public ActionResult ShowGeneric()
        {
            ...
            return RedirectToAction("ShowGenericRpt", "CustomerHistory");
        }
    }
}