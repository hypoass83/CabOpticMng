...
{
    public class CustomerBalanceController : BaseController
    {
        //Current Controller and current page
        private const string CONTROLLER_NAME = "Sale/CustomerBalance";
        ...
        public void Reset()
        {
            this.GetCmp<FormPanel>("RptCustomerBalance").Reset(true);
            this.PartialReset();
        }
        ...
        private void chargeSolde()
        {
            ...
            var lstAccountOperations = db.AccountOperations.Where(ao => ao.BranchID == BranchID && ao.DeviseID == DeviseID && (ao.DateOperation >= Bdate && ao.DateOperation <= Edate) && ao.AccountTierID != null).ToList(); ;
            Totaldebit = lstAccountOperations.Sum(s => s.Debit);
            TotalCredit = lstAccountOperations.Sum(s => s.Credit);
        ...
        public ActionResult ShowGeneric()
        {
            ...
            this.Session["RepTitle"] = Resources.RptCustomerBalanceTitle;
            ...
            return RedirectToAction("ShowGenericRpt", "CustomerBalance");
        }
    }
}