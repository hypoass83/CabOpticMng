...
namespace FatSodDental.UI.Areas.Supply.Controllers
{
    ...
    public partial class StockInputController : BaseController
    {
        private bool isLens = false;
        //Current Controller and current page
        private const string CONTROLLER_NAME = "Supply/StockInput" ;
        ...
        {
            this.GetCmp<Store>("StockInputGridStoreID").Reload();
        }
        [HttpPost]
        public StoreResult GetAllPendingStockInputs(DateTime Bdate, DateTime Edate)
        {
        ...
        private void chargeSolde()
        {
           ...
           //var invHistory = db.InventoryHistorics.Where(ih => ih.InventoryDate <= Edate.Date && ih.ProductID == pl.ProductID && ih.LocalizationID == pl.LocalizationID && ih.StockStatus=="INPUT").OrderByDescending(s => s.InventoryDate).Take(1);
           //foreach(var getHist in invHistory)
           //{
           //    TotalQty = TotalQty + getHist.NewStockQuantity;
           //}

            TotalQty = TotalInPutProductQtyPeriodDate(Bdate, Edate, pl);
           //TotalQty = TotalInPutProductQtyByDate(Edate.Date, pl);
        ...
        private List<object> ModelRptProductSale(DateTime Bdate, DateTime Edate,ProductLocalization pl) 
        {
            ...
            var LstinvHistory = db.InventoryHistorics.Where(ih => ih.InventoryDate >= Bdate.Date && ih.InventoryDate <= Edate.Date && ih.ProductID == pl.ProductID && ih.LocalizationID == pl.LocalizationID && ih.StockStatus == "INPUT").ToList().OrderBy(s => s.InventoryHistoricID);
        ...
        private double TotalInPutProductQtyPeriodDate(DateTime DateDebut, DateTime DateFin, ProductLocalization pl)
        {
            double TotalStockReg = 0d;
            double TotalQty = 0d;
            //recuperation du stock de depart
            var invHistoryStockReg = db.InventoryHistorics.Where(ih => ih.InventoryDate <= DateFin.Date && ih.InventoryDate >= DateDebut.Date && ih.ProductID == pl.ProductID && ih.LocalizationID == pl.LocalizationID && ih.inventoryReason.Trim().ToLower() == "stock regularization").OrderByDescending(s => s.InventoryHistoricID).Take(1);
            foreach (var getStockReg in invHistoryStockReg)
            {
                TotalStockReg = TotalStockReg + getStockReg.NewStockQuantity;
            }

            var invHistory = db.InventoryHistorics.Where(ih => ih.InventoryDate <= DateFin.Date && ih.InventoryDate >= DateDebut.Date && ih.ProductID == pl.ProductID && ih.LocalizationID == pl.LocalizationID && ih.StockStatus.Trim().ToUpper() == "INPUT").ToList();
            foreach (var getHist in invHistory)
            {
                TotalQty = TotalQty + getHist.Quantity;
            }
        ...
        private List<Object> ModelHistoProduct(DateTime BeginDate, DateTime EndDate, ProductLocalization pl)
        {
            ...
            var LstinvHistory = db.InventoryHistorics.Where(ih => ih.InventoryDate >= BeginDate.Date && ih.InventoryDate <= EndDate.Date && ih.ProductID == pl.ProductID && ih.LocalizationID == pl.LocalizationID && ih.StockStatus.Trim().ToUpper() == "INPUT").ToList().OrderBy(s => s.InventoryHistoricID);
        ...
        public ActionResult ShowGeneric()
        {
            ...
            this.Session["ReportName"] = "RptStockInput";
            if (cmpny != null)
            {
            ...
            this.Session["RepTitle"] = Resources.RptStockInput;
            ...
            return RedirectToAction("ShowGenericRpt", "StockInput");
        }
