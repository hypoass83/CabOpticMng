...
namespace FatSodDental.UI.Areas.Supply.Controllers
{
    ...
    public partial class ProductTransmissionController : BaseController
    {
        private bool isLens = false;
        //Current Controller and current page
        private const string CONTROLLER_NAME = "Supply/" + CodeValue.Supply.ProductTransmission_SM.CONTROLLER;
        private const string VIEW_NAME = CodeValue.Supply.ProductTransmission_SM.PATH;
        ...
        public ProductTransmissionController(ITransfert transRepo, 
            IBusinessDay bDRepo,
        ...
        public ActionResult ProductTransmission()
        {
        ...
        public ActionResult AddManager(ProductTransfert transfer)
        {
            ...
            {
                transfer.ProductTransfertDate = SessionBusinessDay(null).BDDateOperation.Date;
            ...
            if (transfer.ProductTransfertID > 0)
            {
                ...
                if (transfer.ProductTransfertDate.Date == SessionBusinessDay(null).BDDateOperation.Date) return UpdateTransfer(transfer);
                else
                {
                    X.Msg.Alert(Resources.er_alert_danger, "You cannot Update this Transfer because the Transfer date is different from the Business Day ").Show();
                    return this.Direct();
                }
            }
        ...
        public ActionResult InitTrnNumber(int? BranchID)
        {
            if (BranchID > 0)
            {
                ...
                string trnnum = _transactNumbeRepository.displayTransactNumber("TRAN", businessDay);
        ...
        [HttpPost]
        public ActionResult DoTransfer(ProductTransfert transfer)
        {
            try
            {

                int productTransmissionID= _transfertRepository.DoTransfert(transfer,SessionGlobalPersonID).ProductTransfertID;
                Session["ProductTransmissionID"] = productTransmissionID;
        ...
        [HttpPost]
        public ActionResult UpdateTransfer(ProductTransfert transfer)
        {
            try
            {
                int productTransmissionID= _transfertRepository.UpdateTransfert(transfer).ProductTransfertID;
                Session["ProductTransmissionID"] = productTransmissionID;
                this.GetCmp<Button>("btnPrint").Disabled = false;
        ...
        public ActionResult AddTransferLine(ProductTransfertLine transferLine)
        {
            ...
            transferLine.DepartureLocalization = db.Localizations.Find(transferLine.DepartureLocalizationID);
            ...
            //verifiction ds le stock source s'il ya assez de produit a transferer
            bool res = _productLocRepository.checkQtyInStock(transferLine.ProductID, transferLine.DepartureLocalizationID.Value, transferLine.LineQuantity);
            if (res)
            {
            ...
            catch (Exception ex)
            {
                ...
                X.Msg.Alert(Resources.er_alert_danger, "Products Have not Been  Successfully Updated because " + ex.Message).Show();
                return this.Direct();
            }
        }
        ...
        public ActionResult UpdateTransfer(int ProductTransfertID)
        {
            ...
            Session["ProductTransmissionID"] = ProductTransfertID;
            this.GetCmp<Button>("btnPrint").Disabled = false;
            Session["ProductTransfertLines"] = ProductTransfertLines;
            this.SimpleReset2();
        ...
        public void InitializeProductTransmissionFields(int ID)
        {
            ...
            ProductTransfert transfer = db.ProductTransferts.Find(ID);
        ...
        public ActionResult ChangeBusDay(int? DepartureBranchID)
        {
            ...
            if (DepartureBranchID.HasValue && DepartureBranchID.Value > 0)
            {
                /* = (List<BusinessDay>)Session["UserBusDays"];
                if (lstBusDay == null)
                {
                    lstBusDay = _busDayRepo.GetOpenedBusinessDay(CurrentUser);
                };
                ...
                this.GetCmp<DateField>("ProductTransfertDate").SetValue(businessDay.BDDateOperation);
                  */
                this.InitTrnNumber(DepartureBranchID.Value);
        ...
        public List<object> ModelDeparturePendingTransfer(DateTime BeginDate,DateTime TransferDate)
        {
            List<object> list = new List<object>();
            ...
            {*/
                ...
                List<ProductTransfert> dataTmp = (from pt in db.ProductTransferts
                                                  where (!pt.IsReceived && (pt.ProductTransfertDate>=BeginDate.Date && pt.ProductTransfertDate <= TransferDate.Date))
                                                      select pt).ToList();
        ...
        //[HttpPost]
        public StoreResult GetAllDeparturePendingTransfers(DateTime BeginDate, DateTime TransferDate)
        {
            return this.Store(ModelDeparturePendingTransfer(BeginDate,TransferDate));
        }
        ...
        public List<Product> ModelProductLocalCat(int? DepartureLocalizationID, int? ProductCategoryID, int? ProductNumberID)
        {
            ...
            if ((DepartureLocalizationID == null || DepartureLocalizationID.Value == 0) && (ProductCategoryID == null || ProductCategoryID.Value == 0) && (ProductNumberID == null || ProductNumberID.Value == 0)) //chargement des produits en fct du magasin slt
            {
                return model;
        ...
        public ActionResult OnProductSelected(int? Localization, int? CurrentProduct)
        {
            ...
            try
            {
                ...
                //ProductLocalization prodLoc = _productLocalizationRepository.FindAll.SingleOrDefault(pl => pl.ProductID == CurrentProduct.Value &&
                //pl.LocalizationID == Localization.Value);

                //1-Quantité en stock
                //this.GetCmp<NumberField>("StockQuantity").Value = (double)Session["MaxValue"];// prodLoc.ProductLocalizationStockQuantity;
                //2-Quantité maximale pouvant être vendue
                //Session["MaxValue"] = prodLoc.ProductLocalizationStockQuantity;
                //prodLoc.ProductLocalizationStockQuantity
        ...
        public void GenerateReceipt()
        {
            ...
            try
            {
            int productTransmissionID = (int)Session["ProductTransmissionID"];
            ...
            ProductTransfert productTransfert = (from pt in db.ProductTransferts
                                                 where pt.ProductTransfertID == productTransmissionID
                                   select pt).SingleOrDefault();
                //_transfertRepository.FindAll.Where(pt => pt.ProductTransfertID == productTransmissionID).SingleOrDefault();
            ...
            db.ProductTransfertLines.Where(l => l.ProductTransfertID == productTransmissionID).ToList().ForEach(c =>
            {
                isValid = true;
                ...
                model.Add(
                                new
                                {
                                    ...
                                    TransfertDate = productTransfert.ProductTransfertDate.Date,
