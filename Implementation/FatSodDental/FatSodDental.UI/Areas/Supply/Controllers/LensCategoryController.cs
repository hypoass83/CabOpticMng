...
namespace FatSodDental.UI.Areas.Supply.Controllers
{
    ...
    public class LensCategoryController : BaseController
    {
        ...
        public ActionResult LensCategory(LensCategory category)
        {
        ...
        public ActionResult AddCategory(LensCategory category)
        {
            try
            {
                ...
                else
                {
                    ...
                    X.Msg.Alert(Resources.b_LensCategory, statusOperation).Show();
        ...
        /// <returns></returns>
        ////[HttpPost]
        public ActionResult UpdateCategory(LensCategory category)
        {
            try
            {
        ...
        public ActionResult DeleteCategory(int CategoryID)
        {
            try
            {
                ...
                {
                    X.Msg.Alert("UnDiletable LensCategory", "You can Not Delete this LensCategory").Show();
                    return this.Direct();
                }
                ...
                if ((categoryProducts != null) || (categoryProducts.Count != 0))
                {
                    ...
                    db.SaveChanges();
                    //lensCategoryRepository.Delete(deletedCategoy);
                    //TempData["status"] = "The LensCategory " + deletedCategoy.CategoryCode + " has been successfully deleted";
                    //TempData["alertType"] = "alert alert-success";
                    X.Msg.Alert("LensCategory", "The LensCategory " + deletedCategoy.CategoryCode + " has been successfully deleted").Show();
                    return this.Reset();
                }
                else
                {
                    ...
                    X.Msg.Alert("LensCategory", @"Désolé vous ne pouvez pas supprimer la catégorie " + deletedCategoy.CategoryCode + " parcequ'elle contient déjà des produits"
            ...
            catch (Exception e)
            {
                ...
                X.Msg.Alert("LensCategory", @"L'erreur " + e.Message + " s'est produite lors de la suppression! veuillez contactez l'administrateur et/ou essayez à nouveau").Show();
                return this.Direct();
            }
        }
        ...
        public void InitializePurchaseFields(int ID)
        {
            ...
            if (ID > 0)
            {
                ...
                this.GetCmp<TextField>("SupplyingName").Value = (category.SupplyingName != null && category.SupplyingName.Length > 0) ? category.SupplyingName : category.CategoryCode;
                this.GetCmp<TextField>("LensMaterialID").Value = category.LensMaterialID;
                this.GetCmp<TextField>("LensColourID").Value = category.LensColourID;
                this.GetCmp<TextField>("LensCoatingID").Value = category.LensCoatingID;
                this.GetCmp<TextArea>("CollectifAccountID").Value = category.CollectifAccountID;

                this.GetCmp<TextField>("LensIndex").Value = category.LensIndex;
                this.GetCmp<NumberField>("LensDiameter").Value = category.LensDiameter;

                this.GetCmp<ComboBox>("TypeLens").Value = category.TypeLens;
            }
        }
